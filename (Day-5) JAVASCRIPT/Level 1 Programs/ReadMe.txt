...
to declare the variable we can either let or var
 ...
 The DOM is a programming interface that allows js to access, change add or remove html elements on a web page
 HTML page is converted into a tree structure that java script can control

 HTML is static (all alone we cannot change by itself)
...
fetch is a built-in api that return apromise object for http calls and new Promise() is used to manually create and then 
returning an object

before using arrow function we are using a normal function

function add(a,b)          jsonData.forEach(function(product,index){document.write(product,index)})
{
	return a+b;
...
jsonData.forEach((product,index)=>
{
	...
	li.textContent=`Id: $(index) | Item: ${product:item}
})